...
void to_string(vec* v) {
    ...
    char buffer[26]; // 24 char bytes for float, 1 char byte for comma, 1 char byte for null terminator
    printf("[");
    for(int i = 0; i < elem_size - 1; i++) {
        snprintf(buffer, 26, "%f,", elems[i]); // load buffer
